...
void brakeAllMotors(int brakeTimeMs = 30) {
  // Phase 1: Set all motors to braking state first
  for (int i = 0; i < 4; i++) {
    digitalWrite(motorPins[i][2], reverse[i]); // Set direction pin
  }

  // Apply full PWM after all directions are set
  for (int i = 0; i < 4; i++) {
    analogWrite(motorPins[i][1], 255); // Engage short braking (full duty)
  }

  // Phase 2: Hold braking state
  delay(brakeTimeMs);

  // Phase 3: Turn off all motors together
  for (int i = 0; i < 4; i++) {
